...
exports.uploadAndCheckPathIMG = async(userId, image) => {
    try {
        ...
        const imagePath = fsPromises.readFile(image.path);
        const uploadDir = `public/candidate/${userId}`;
        const uploadFileName = `${timestamp}_${image.originalFilename}.jpg`;
        const uploadPath = path.join(uploadDir, uploadFileName);
        fsPromises.mkdir(uploadDir, { recursive: true });
        fsPromises.writeFile(uploadPath, imagePath);
        fsPromises.access(uploadPath);
