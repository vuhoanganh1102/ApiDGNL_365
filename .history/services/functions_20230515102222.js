...
exports.getDataCVSortById = async(condition) => {
    const data = await CV.find(condition).select('_id image name price status view love download lang_id design_id cate_id colors').sort('-_id');
    if (data.length > 0) {
        return data;
    };
    return null;
...
exports.getDataCVSortByDownload = async(condition) => {
    const data = await CV.find(condition).select('_id image name price status view love download lang_id design_id cate_id colors').sort('-download');
    if (data.length > 0) {
        return data;
    };
    return null;
...
exports.getDataDonThu = async(model, condition) => {
    const data = await model.find(condition).select('_id name price image').sort('-_id');
    if (data.length > 0) {
        return data;
    };
    return null;
...
exports.base64Encoded = async(req, res, next) => {
    const base64Encoded = req.body.nameImage;
    req.body.nameImage = Buffer.from(base64Encoded, 'base64').toString('utf-8');
    return next();
