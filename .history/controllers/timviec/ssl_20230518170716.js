...
exports.findByKeyword = async(req, res, next) => {
    try {
        const keyword = req.body.keyword || 'ToànQuốc';

        const nganh = await newTV365.find({ cityID: { $in: [`${keyword}`] }, title: { $regex: '', $options: 'i' } }).select('newMoney.minValue newMoney.maxValue');
        let tl = 0;
        let value = [];
        nganh.map(obj => {
