...
exports.detailCV = async(req, res, text) => {
    try {
        ...
        const data = await CV.findOne({ _id: _id }).select(`_id name ${html_lang} view`);
...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        var dateString = year.toString() + '-' + month.toString().padStart(2, '0') + '-' + day.toString().padStart(2, '0');
