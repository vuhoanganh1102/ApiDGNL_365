...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        const data = req.body;
        const checkAvatar = await functions.checkImage(imageFile.path);

        if (checkAvatar == false) return functions.setError(res, 'Lỗi ảnh', 404);

        if (await functions.uploadAndCheckPathIMG(userId, imageFile) != 'EXIT') return await functions.setError(res, 'Upload ảnh thất bại', 404);
