...
exports.countPoints = async(req, res, next) => {
    try {
        ...
        if (cv) {
            let points = await cv.cvPoint + point;
            console.log(points);
            const data = await CV.findOneAndUpdate({ _id }, { cvPoint: points });
            if (data) return await functions.success(res, 'Cập nhật điểm cv thành công', data);
        }
