...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        const base64Encoded = Buffer.from(data.nameImage).toString('base64');
        const nameImage = Buffer.from(base64Encoded, 'base64').toString('utf-8');



        const checkAvatar = await functions.checkImage.single(nameImage.path);
