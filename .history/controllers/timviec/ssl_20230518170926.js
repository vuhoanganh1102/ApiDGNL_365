...
exports.findByKeyword = async(req, res, next) => {
    try {
        const cityID = req.body.keyword || 'ToànQuốc';
        const keyword = req.body.keyword;

        const nganh = await newTV365.find({ cityID: { $in: [`${cityID}`] }, title: { $regex: `${keyword}`, $options: 'i' } }).select('newMoney.minValue newMoney.maxValue');
        let tl = 0;
        let value = [];
        nganh.map(obj => {
