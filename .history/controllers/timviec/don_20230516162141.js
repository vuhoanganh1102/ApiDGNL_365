...
exports.createDonXinViec = async(req, res, next) => {
    try {
        const user = req.user.data;
...
exports.findDonXinViec = async(req, res, next) => {
    try {
        const user = req.user.data;
        if (user.role != 1) return await functions.setError(res, 'Chưa có quyền truy cập');
...
exports.updateDonXinViec = async(req, res, next) => {
    try {
        const user = req.user.data;
        if (user.role != 1) return await functions.setError(res, 'Chưa có quyền truy cập');
        const _id = req.params._id;
...
//xóa mẫu DonXinViec
exports.deleteDonXinViec = async(req, res, next) => {
    try {
        const user = req.user.data;
        if (user.role != 1) return await functions.setError(res, 'Chưa có quyền truy cập');
        const _id = req.params._id;
