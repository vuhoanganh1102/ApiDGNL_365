...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        const cv_name = getValue('cv_name', 'str', 'POST', '');
        const cv_id = getValue('cv_id', 'int', 'POST', 0);

        if (userId !== 0 && uploadImage.nameImage !== '' && cv_id !== 0) {
            const db_qr = new db_query(`SELECT chat365_id FROM user WHERE use_id = ${userId} LIMIT 1`);
            const row = await db_qr.result;

            if (row.length > 0) {
                if (row[0].chat365_id !== 0) {
                    const senderId = 1191;
                    const text = '';
                    const linkImg = `https://timviec365.vn/cv365/upload/ungvien/uv_${userId}/${uploadImage.nameImage}`;
                    const linkPdf = `https://timviec365.vn/cv365/download-cvpdf/cv.php?cvid=${cv_id}&uid=${userId}&cvname=${uploadImage.nameImage}`;

                    const data = {
                        userId: row[0].chat365_id,
                        senderId: senderId,
                        linkImg: linkImg,
                        linkPdf: linkPdf,
                        Title: text,
