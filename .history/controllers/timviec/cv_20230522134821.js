...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        // Thiết lập kích thước và vị trí hiển thị ảnh trong PDF
        const pdfImageWidth = 612; // Chiều rộng trang PDF
        const pdfImageHeight = (pdfImageWidth / width) * height;

        doc.image(image, 0, 0, { width: pdfImageWidth, height: pdfImageHeight });

        // Kết thúc ghi dữ liệu vào PDF
        doc.end();

        // const chatId = await chat365.findOne({ chat365Id });

        // if (chatId) {
        //     if (chatId.chat365_id !== 0) {
        //         const senderId = 1191;
        //         const text = '';
        //         const linkPdf = `https://timviec365.vn/cv365/download-cvpdf/cv.php?cvid=${data._id}&uid=${userId}&cvname=${uploadImage.nameImage}`;
