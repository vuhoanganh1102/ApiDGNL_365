...
exports.createDon = async(req, res, next) => {
    try {
        ...
        data._id = _id;
        await Don.create(data);
        return await functions.success(res, 'Tạo mới Don thành công', );
    } catch (err) {
        return functions.setError(res, err.message);
    };
};

// lấy dữ liệu mẫu Don cũ
exports.findDon = async(req, res, next) => {
    try {
        ...
        const _id = req.params._id;
        const data = await Don.findOne({ _id: _id });
...
// update dữ liệu mẫu Don
exports.updateDon = async(req, res, next) => {
    try {
        const user = req.user.data;
        if (user.role != 1) return await functions.setError(res, 'Chưa có quyền truy cập');
        const _id = req.params._id;
        const data = await Don.findOneAndUpdate({ _id: _id }, req.body);
...
exports.deleteDon = async(req, res, next) => {
    try {
        ...
        if (user.role != 1) return await functions.setError(res, 'Chưa có quyền truy cập');
        const _id = req.params._id;
        const data = await Don.findOneAndDelete({ _id: _id });
