...
exports.findCompany = async(req, res, next) => {
    try {
        ...
        if (diadiem == 0) { data = Users.find({ type: 1 }, { 'inForCompany.tagLinhVuc': { $regex: keyword } }) };
        else { data = Users.find({ type: 1 }, { city: diadiem }, { 'inForCompany.tagLinhVuc': { $regex: keyword } }) }
