...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        const u_id = getValue('u_id', 'int', 'POST', 0);
        const cv_name = getValue('cv_name', 'str', 'POST', '');
        const cv_id = getValue('cv_id', 'int', 'POST', 0);

        if (u_id !== 0 && cv_name !== '' && cv_id !== 0) {
            const db_qr = new db_query(`SELECT chat365_id FROM user WHERE use_id = ${u_id} LIMIT 1`);
            ...
            if (row.length > 0) {
                if (row[0].chat365_id !== 0) {
                    const senderId = 1191;
                    const text = '';
                    const linkImg = `https://timviec365.vn/cv365/upload/ungvien/uv_${u_id}/${cv_name}.png`;
                    const linkPdf = `https://timviec365.vn/cv365/download-cvpdf/cv.php?cvid=${cv_id}&uid=${u_id}&cvname=${cv_name}`;
