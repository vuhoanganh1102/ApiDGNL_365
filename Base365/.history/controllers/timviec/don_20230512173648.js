...
exports.getByNganh = async(req, res, next) => {
    try {
        ...
        if (data) return await functions.success(res, `Đơn theo ngành ${cateId}`, data);
...
exports.getNganhDon = async(req, res, next) => {
    try {
        ...
        if (data.length > 0) return functions.success(res, 'Danh sách ngành đơn', data);
...
exports.previewDon = async(req, res, next) => {
    try {
        ...
        return await functions.success(res, 'Lấy mâũ đơn thanh công', data);
    } catch (err) {
        return functions.setError(res, err.message);
    };
};
...
exports.detailDon = async(req, res, next) => {
    try {
        ...
        const data = await DonXinViec.findOne({ _id: _id }).select('_id name htmlVi htmlCn htmlJp htmlKr htmlEn view');
