...
exports.detailCV = async(req, res, text) => {
    try {
        ...
        const data = await CV.findOne({ _id: _id }).select(`_id name ${html_lang} view cate_id color`);
...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        // const base64Encoded = data.nameImage;
        const base64Encoded = Buffer.from('Ban mai-Thanh Leanh.jpg').toString('nameImage');
        console.log(base64Encoded, 1);
        const text = Buffer.from(base64Encoded, 'nameImage').toString('utf-8');
