...
exports.saveCV = async(req, res, next) => {
    try {
        ...
        function encodeObjectToBase64(obj) {
            ...
            const bufferObj = Buffer.from(jsonString, 'utf-8');
            const encodedObj = bufferObj.toString('base64');
            return encodedObj;
        }

        // Đối tượng cần mã hóa
        const myObject = {
            fieldname: 'base64Encoded',
            originalname: 'Ban mai-Thanh Leanh.jpg',
            encoding: '7bit',
            mimetype: 'image/jpeg',
            destination: 'public/KhoAnh/2',
            filename: 'base64Encoded1684199146463-352767461.jpg',
            path: 'public\\KhoAnh\\2\\base64Encoded1684199146463-352767461.jpg',
            size: 206102,
        };

        // Mã hóa đối tượng thành Base64
        const encodedObject = encodeObjectToBase64(myObject);
        console.log(encodedObject, 2)
        const nameImage = req.files;
