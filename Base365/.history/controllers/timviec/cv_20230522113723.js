        ...
        if (userId !== 0 && cv_name !== '' && cv_id !== 0) {
            ...
            if (row.length > 0) {
                if (row[0].chat365_id !== 0) {
                    const senderId = 1191;
                    const text = '';
                    const linkImg = `https://timviec365.vn/cv365/upload/ungvien/uv_${userId}/${cv_name}.png`;
                    const linkPdf = `https://timviec365.vn/cv365/download-cvpdf/cv.php?cvid=${cv_id}&uid=${userId}&cvname=${cv_name}`;
