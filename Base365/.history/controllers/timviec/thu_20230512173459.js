...
exports.getByNganh = async(req, res, next) => {
    try {
        ...
        if (data) return await functions.success(res, `Thư theo ngành ${cateId}`, data);
...
exports.getNganhThu = async(req, res, next) => {
    try {
        ...
        if (data.length > 0) return functions.success(res, 'Danh sách ngành thư', data);
...
exports.previewThu = async(req, res, next) => {
    try {
        ...
        return await functions.success(res, 'Lấy mâũ thư thanh công', data);
    } catch (err) {
        return functions.setError(res, err.message);
    };
};
