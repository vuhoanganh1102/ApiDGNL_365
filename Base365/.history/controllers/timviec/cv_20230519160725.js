    ...
    try {
        ...
        if (sort == 1) {
            if (cateId || langId || designId) {
                if (cateId) {
                    data = await functions.getDataCVSortByDownload({ cateId: cateId });
                    return await functions.success(res, 'Lấy dữ liệu thành công', data);
                };
                if (langId) {
                    data = await functions.getDataCVSortByDownload({ langId: langId });
                    return await functions.success(res, 'Lấy dữ liệu thành công', data);
                };
                if (designId) {
                    data = await functions.getDataCVSortByDownload({ designId: designId });
                    return await functions.success(res, 'Lấy dữ liệu thành công', data);
                };
            };
            ...
            if (cateId) {
                data = await functions.getDataCVSortById({ cateId: cateId });
                return await functions.success(res, 'Lấy dữ liệu thành công', data);
            };
            if (langId) {
                data = await functions.getDataCVSortById({ langId: langId });
                return await functions.success(res, 'Lấy dữ liệu thành công', data);
            };
            if (designId) {
                data = await functions.getDataCVSortById({ designId: designId });
                return await functions.success(res, 'Lấy dữ liệu thành công', data);
            };
...
exports.previewCV = async(req, res, next) => {
    try {
        ...
        const data = await CV.findOne({ _id }).select('_id lang_id name image mota_cv colors view ');
