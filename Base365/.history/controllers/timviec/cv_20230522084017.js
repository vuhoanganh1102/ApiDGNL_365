...
exports.saveCV = async(req, res, next) => {
        try {
            ...
            if (checkAvatar == false) return functions.setError(res, 'Lỗi ảnh', 404);
            ...
            const check = function checkFileExistence(filePath) {
                try {
                    fs.access(filePath);
                    return 'EXIT'
                } catch (error) {
            ...
            console.log(functions.uploadAndCheckPathIMG(userId, nameImage) if (check(uploadPath) != 'EXIT') {
                    return await functions.setError(res, 'Upload ảnh thất bại', 404)
                }
                ...
                let _id = 1; await functions.getMaxID(CVUV)
                .then(res => {
                    if (res) {
                        _id = res + 1;
                    }
                ...
                cvUV._id = _id;
                const newCVUV = await CVUV.create(cvUV); console.log(newCVUV);
                if (newCVUV) {
