...
exports.getDataCVSortById = async(condition) => {
    const data = await CV.find(condition).select('_id image name alias price status view love download langId designId cateId colors').sort({ _id: -1 });
    if (data.length > 0) {
        return data;
    };
    return null;
...
exports.getDataCVSortByDownload = async(condition) => {
    const data = await CV.find(condition).select('_id image name alias price status view love download langId designId cateId colors').sort({ download: -1 });
    if (data.length > 0) {
        return data;
    };
    return null;
