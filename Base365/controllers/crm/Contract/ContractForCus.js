...
exports.showContract = async (req,res) =>{
    const {com_id, _id, id_customer} = req.body;
    if((com_id&&id_customer) == undefined){
        functions.setError(res,"thieu thong tin")
    }else{
        const data = await crm_contract.find({id_customer:id_customer,com_id :com_id},{id_form_contract :1 ,user_created:1,status:1,created_at:1})
        if(data == undefined){
            functions.setError(res,"khong có hợp đồng nào từng được tạo cho khách hàng này")
...
exports.showDetailContract = async (req, res) => {
    ...
    const {  _id, id_customer,status,id_form_contract,emp_id,path_dowload ,is_delete} = req.body;
    let list = [];
...
exports.deleteContract = async (req,res) =>{
    const {_id , com_id, id_customer} = req.body;
    const data = await crm_contract.findOne({_id:_id,com_id:com_id,id_customer:id_customer})
    if(!data){
        functions.setError(res," hop dong k ton tai ")
    }else{
       const result = await crm_contract.findOneAndUpdate({_id:_id,com_id:com_id,id_customer:id_customer},{$set : {is_delete : 1}})
