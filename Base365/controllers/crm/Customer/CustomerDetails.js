            ...
            if (checkToken) {
              ...
              if (info_extension.err_code !== undefined) {
                ...
                let checkCalled = await CallHistory.findOne({
                  phone: phone,
                  extension: infor.ext_number,
                  com_id: com_id // Điều này cần được định nghĩa
                });
                if (!checkCalled) {
...
exports.QuanLyLine = async (req, res) => {
  try {
      ...
      if (result) {
          ...
          const totalRecords = ist_extension_paginate.extensions.length;
          const limit = 15;
          const totalPages = Math.ceil(totalRecords / limit);

          const page = req.query.page || 1; // Truy cập số trang từ query parameters
          const startIndex = (page - 1) * limit;
          const endIndex = page * limit;

          const paginatedList = ist_extension_paginate.extensions.slice(startIndex, endIndex);

          let data = {
              list_Line: paginatedList,
